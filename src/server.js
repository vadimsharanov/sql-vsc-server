...
import { deletePeople, getOnePeople, getPeople, savePeople } from "./db.js";
...
app.get("/zmogus/:id?", async (req, res) => { // sukuriame atskira puslapy, kiekvienam zmogui
    try {
        let zmogus = null;
        if (req.params.id) {
        zmogus = await getOnePeople(req.params.id)
}
...
app.get("/zmogus/:id/delete", async (req, res) => { // padarom linka, i kuri nuejus zmogus trinamas
    try {
